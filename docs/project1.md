# Entregable 1 - Wearable de Seguridad

Realizado por Nicolás Villanueva.

## 1. Revisión de Funcionalidad

El prototipo realizado consiste en una muñequera _wearable_ que detecta exposiciones a niveles elevados de luminosidad, y activa una alarma cuando transcurre cierto período de tiempo a dicha exposición. Integra otras funcionalidades como el titilar de un LED verde previo a la activación de la alarma, el establecimiento de un período de reposo tras desactivar la alarma, y la detección del giro de dicha muñequera.

El proyecto se divide en dos entregas incrementales, cuyos requisitos (mencionados en el documento del enunciado) se pueden sintetizar en los puntos a continuación. Hay algunas decisiones de diseño que, aunque no establecidas explícitamente en el documento, han sido tomadas por el alumno y resaltadas **en negrita**

### Entregable 1:
* El wearable debe mostrar el nivel de luminosidad relativo al que esté expuesto mediante la barra de LEDs del microcontrolador
* Se debe contar el tiempo transcurrido a partir de cierto umbral de exposición luminosa (establecida en tiempo de compilación):
* A los 10 segundos, se debe de activar el titilar del LED verde del microcontrolador
* Si el nivel de exposición baja hasta ser inferior al umbral, se desactiva el titilar del LED verde
* A los 20 segundos, se debe de activar el zumbador como alarma
* Si el nivel de exposición baja hasta ser inferior al umbral, NO se desactiva la alarma, **aunque sí el titilar del LED verde**
* Para desactivar la alarma se debe pulsar el pulsador, activando el modo reposo
* El modo reposo durará 30 segundos, donde no se realizará ninguna medida y **no se activarán la barra de LEDs, el titilar del LED verde ni la alarma del zumbador**.

### Entregable 2:
* Se integrará al wearable un sensor de giro (Acelerometro) que pasará al estado "activo" al girar la muñequera hacia el interior, y al estado "inactivo" al girarla hacia el exterior
* Al encontrarse en estado "inactivo", **La barra de LEDs no se encenderá**, pero sí se detectarán los niveles de exposición luminosa y **podrán activarse el titilar del LED verde y la alarma del zumbador**.
